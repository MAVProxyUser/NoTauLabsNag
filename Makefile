...
.PHONY: androidgcs
androidgcs: uavobjects_java uavo-collections_java
...
androidgcs_clean:
	$(V0) @$(ECHO) " CLEAN      $@"
	$(V1) [ ! -d "$(ANDROIDGCS_OUT_DIR)" ] || $(RM) -r "$(ANDROIDGCS_OUT_DIR)" ||  $(RM) -r ${ROOT_DIR}/androidgcs/assets/uavos/*.jar
...
$$(UAVO_COLLECTION_DIR)/$(1)/java-build/uavobjects.jar: $$(UAVO_COLLECTION_DIR)/$(1)/java-build/java
	...
	$$(V1) ( \
		...
		cd $$(UAVO_COLLECTION_DIR)/$(1)/java-build && \
		$(JAVAC) java/*.java \
			$$(ROOT_DIR)/androidgcs/src/org/openpilot_nonag/uavtalk/UAVDataObject.java \
			$$(ROOT_DIR)/androidgcs/src/org/openpilot_nonag/uavtalk/UAVObject*.java \
			$$(ROOT_DIR)/androidgcs/src/org/openpilot_nonag/uavtalk/UAVMetaObject.java \
			-d . && \
		find ./org/openpilot_nonag/uavtalk/uavobjects -type f -name '*.class' > classlist.txt && \
		$(JAR) cf tmp_uavobjects.jar @classlist.txt && \
		$$(ANDROID_DX) \
			--dex \
			--output $$(ANDROIDGCS_ASSETS_DIR)/uavos/$$$${HASH}.jar \
			tmp_uavobjects.jar && \
		cp $$(ANDROIDGCS_ASSETS_DIR)/uavos/$$$${HASH}.jar ${ROOT_DIR}/androidgcs/assets/uavos \
	)
	# the cp call is a hack to fix the .jar file misisng from the apk which compiled via Makefile + make
endef
#		mkdir -p ${ROOT_DIR}/androidgcs/assets/uavos \
